<?php
  ...
  function ageInTheFuture($dob,$future) {
    $dob = explode("/", $dob);

    $future = explode("/", $future);

    $futureAge = $future[2] - $dob[2];

    if($dob[0]<$future[0] || ($dob[0] == $future[0] && $dob[1]<$future[1]) ){
        $futureAge--;
    }
...
<html lang="en">
  ...
  <body>
    <main class="container py-4">
      ...
      <?php echo '<p>Your actual age is '.getAge('03/27/1995').'.</p>'; ?>
      
      <?php echo '<p> This means on July 23, 2054 you\'ll be ' .ageInTheFuture('03/27/1995','07/23/2054').'.</p>'; ?>
      ...
      <p><strong> The fourth <code>$sum4</code> variable is:</strong>
      <?php echo $sum4; ?></p>
